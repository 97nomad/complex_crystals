...
impl MainMenuScene {
    pub fn new(window: &mut PistonWindow) -> Self {
        ...
        let mut ui = ::conrod::UiBuilder::new([800.0, 600.0])
            .theme(::conrod::Theme {
        ...
        let (glyph_cache, text_texture_cache) = {
            ...
            let cache =
                ::conrod::text::GlyphCache::new(800, 600, SCALE_TOLERANCE, POSITION_TOLERANCE);
            let buffer_len = 800 * 600;
            let init = vec![128; buffer_len];
            let settings = TextureSettings::new();
            let factory = &mut window.factory;
            let texture = G2dTexture::from_memory_alpha(factory, &init, 800, 600, &settings)
                .unwrap();
            (cache, texture)
        };
...
impl Scene for MainMenuScene {
    fn render(&mut self, window: &mut PistonWindow, args: RenderArgs) {
        window.draw_2d(&Input::Render(args), |context, graphics| {
            // clear([0.8, 1.0, 0.8, 1.0], graphics);
    ...
    fn update(&mut self, _: &UpdateArgs) -> SceneAction {
        ...
        widget::Text::new("Hello, Deer")
            .mid_top_of(self.ids.canvas)
            .set(self.ids.some_text, &mut ui);
